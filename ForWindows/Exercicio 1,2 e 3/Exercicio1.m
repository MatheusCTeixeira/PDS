...
f1 = 1000/fs*pi; % freqüência dentro da banda de passagem
f2 = 3000/fs*pi; % freqüência dentro da banda de rejeição
den = 1; % como o sistema é do tipo FIR O Denominador Da TF é 1.

t = 0:1:1000; %Tempo

sinal_ruido = 4*sin((2*f1*t))+ sin((2*f2*t)); %Sinal com o Ruido
...
filtro_hamming = fir1(L,fc,'low',janela_hamming);
filtragem_hamming = filter(filtro_hamming,den,sinal_ruido);

%Filtragem Blackman
janela_blackman = blackman(L+1);
filtro_blackman = fir1(L,fc,'low',janela_blackman);
